...
export class JioTV {
    ...
    static async requestOTP(number) {
        ...
        // Jio Web API often REQUIRES the + sign. Mobile API often forbids it.
        const webNumber = `+91${cleanNumber}`;
        const mobileNumber = `91${cleanNumber}`;

        const indianIps = [
            `49.34.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
            `103.211.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
            `157.35.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`
        ];
        const randomIp = indianIps[Math.floor(Math.random() * indianIps.length)];

        const strategies = [
            {
                // STRATEGY 1: Web API with +91 (Most likely to work for Web Key)
                ...
                body: { number: webNumber, otpType: 'sms' }
            },
            {
                // STRATEGY 2: Web API with 'identifier' instead of 'number'
                url: 'https://api.jio.com/v3/users/login/otp/send',
                headers: {
                    'x-api-key': 'l7xx938b6684ee9e4bbe8831a9a682b8e19f',
                    'origin': 'https://www.jiotv.com',
                ...
                body: { identifier: webNumber, otpType: 'sms' }
            },
            {
                // STRATEGY 3: Mobile API with 91 (No +)
                ...
                body: { number: mobileNumber, otpType: 'sms' }
            }
        ];
        ...
        for (const strat of strategies) {
            try {
                ...
                try {
                    ...
                    if (res.ok && !data.error && !data.message?.includes('fail')) return data;
        ...
        return { error: `JioTV Handshake Failed: ${lastError}` };
    }
