    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.startsWith('#EXTHTTP:')) {
            try {
                ...
                // Look for cookie
                const cookie = json.cookie || json.Cookie;
                if (cookie) {
                    // Extract __hdnea__ from cookie string if it's a long string
                    // or just use the whole thing if it looks like the token
                    if (cookie.includes('__hdnea__=')) {
                        const token = cookie.split('__hdnea__=')[1].split(';')[0];
                        if (!currentItem.headers) currentItem.headers = {};
                        currentItem.headers['Cookie'] = cookie; // Save full cookie in headers
                        currentItem.cookieToken = token; // Temporary field to append to URL
                    }
                }
            } catch (e) {
        ...
        if (line.startsWith('#KODIPROP:')) {
            ...
            if (eqIndex !== -1) {
                ...
                if (key === 'inputstream.adaptive.license_type') {
                    ...
                    if (clearKeyMatch) {
                        ...
                        // Also try to extract from URL params (e.g. keyid=...&key=...)
                        try {
            ...
            // Append cookie token if found in EXTHTTP
            if (currentItem.cookieToken && !currentItem.url.includes('__hdnea__=')) {
                const separator = currentItem.url.includes('?') ? '&' : '?';
                currentItem.url = `${currentItem.url}${separator}__hdnea__=${currentItem.cookieToken}`;
            }
