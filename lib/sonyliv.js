...
export class SonyLiv {
    ...
    static async requestOTP(number) {
        ...
        const indianIps = [
            `49.36.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
            `103.213.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
            `157.37.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`
        ];
        const randomIp = indianIps[Math.floor(Math.random() * indianIps.length)];

        const paths = [
            'AGL/1.6/A/ENG/WEB/IN/HR/CREATEOTP-V2',
            'AGL/1.4/A/ENG/WEB/IN/HR/CREATEOTP-V2'
        ];
        ...
        for (const path of paths) {
            try {
                const url = `https://apiv2.sonyliv.com/${path}`;
        ...
        return { error: `SonyLiv Handshake Failed: ${lastError}` };
    }
    ...
    static async verifyOTP(number, otp) {
        ...
        const paths = [
            'AGL/1.6/A/ENG/WEB/IN/HR/VERIFYOTP-V2',
            'AGL/1.4/A/ENG/WEB/IN/HR/VERIFYOTP-V2'
        ];
        ...
        for (const path of paths) {
            try {
                ...
                if (res.ok && data.resultCode === 'OK') {
                    const session = {
                        number: cleanNumber,
                        authToken: data.resultObj.userIdentity,
        ...
        return { error: `OTP Verification Failed: ${lastError}` };
    }
